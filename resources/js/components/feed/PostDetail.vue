...
const props = withDefaults(
    defineProps<{ post: PostItem; focusComments?: boolean; closable?: boolean; contained?: boolean }>(),
    { contained: true },
);
...
<template>
    <div
        ...
        :class="[
            post.media.length ? 'md:grid-cols-[minmax(0,1.25fr)_minmax(360px,1fr)]' : 'md:grid-cols-1',
            contained ? 'h-full min-h-0 overflow-hidden' : '',
        ]"
    >
        <!-- Fotos y videos: en computadora, columna oscura a la izquierda -->
        <div v-if="post.media.length" class="hidden bg-black md:flex md:items-center">
            <MediaCarousel :media="post.media" aspect="h-full w-full" controls fit="contain" />
        </div>

        <div class="flex min-h-0 flex-1 flex-col">
            <header class="border-border flex flex-none items-center gap-3 border-b px-4 py-3">
                <Link :href="businessShow(post.business.slug)" class="flex min-w-0 flex-1 items-center gap-3" @click="emit('close')">
                    <BusinessAvatar :src="post.business.avatar" :name="post.business.name" />
                    <span class="min-w-0">
                        <span class="flex items-center gap-1 font-bold">
                            <span class="truncate">{{ post.business.name }}</span>
                            <BadgeCheck v-if="post.business.verified" class="text-brand size-4 flex-none" />
                        </span>
                        <span class="text-muted-foreground block truncate text-xs">{{ post.business.zone }} · {{ post.ago }}</span>
                    </span>
                </Link>
                <button
                    v-if="closable"
            ...
            <div
                ...
                :class="contained ? 'flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain' : ''"
            >
                <div class="flex-none px-4 pt-3 pb-2">
                    <h2 class="font-display text-xl leading-snug font-bold">{{ post.title }}</h2>
                    <p v-if="post.body" class="text-foreground/90 mt-1.5 text-[15px] leading-relaxed whitespace-pre-line">{{ post.body }}</p>
                </div>

                <MediaCarousel v-if="post.media.length" class="flex-none md:hidden" :media="post.media" aspect="aspect-square" controls />

                <ReactionBar class="flex-none" :post="post" @comment="thread?.focus()" />
