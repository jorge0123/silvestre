...
const props = defineProps<{ url: string; value: string; placeholder: string; extra?: Record<string, string | null> }>();
...
watch(q, (value) => {
    ...
    timer = setTimeout(() => {
        const query = Object.fromEntries(
            Object.entries({ ...props.extra, q: value.trim() }).filter(([, v]) => v !== null && v !== ''),
        );
        router.get(props.url, query, { preserveState: true, preserveScroll: true, replace: true });
    }, 300);
});
