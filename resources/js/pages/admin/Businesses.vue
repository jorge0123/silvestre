...
const tabs = computed(() => [
    ...
    {
        key: 'fundadores' as const,
        label: 'Fundadores',
        count: props.counts.founders,
    },
    {
        key: 'nuevos' as const,
        label: 'Con límites',
        count: props.counts.all - props.counts.founders,
    },
]);
...
function filterBy(key: string | null): void {
    router.get(
        businessesIndex().url,
        Object.fromEntries(
            Object.entries({ q: props.filters.q, filtro: key }).filter(
                ([, v]) => v,
            ),
        ),
        {
...
function openAssign(row: Row): void {
    ...
    form.clearErrors();
    form.plan_id =
        props.plans.find((p) => p.code === 'pro')?.id ??
        props.plans[0]?.id ??
        null;
}
...
function toggleFounder(): void {
    ...
    router.post(
        founderRoute(founderTarget.value.id).url,
        {},
        {
            preserveScroll: true,
...
<template>
    ...
    <div class="mx-auto flex w-full max-w-5xl flex-col gap-5 px-4 py-5 md:py-8">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            <SearchBox
                :url="businessesIndex().url"
                :value="filters.q"
                :extra="{ filtro: filters.filtro }"
                placeholder="Buscar por nombre, @usuario o correo"
                class="flex-1"
            />
            <div class="flex [scrollbar-width:none] gap-1.5 overflow-x-auto">
                <button
                    v-for="t in tabs"
                    :key="t.label"
                    type="button"
        ...
        >
            ...
            >
                <div class="flex min-w-0 flex-1 basis-64 items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <p
                            class="flex flex-wrap items-center gap-x-2 font-bold"
                        >
                            <span class="truncate">{{ b.name }}</span>
                            <span
                                v-if="b.founder"
                                class="bg-ok-soft text-ok inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-bold"
                            >
                ...
                <div class="min-w-32 text-sm">
                    ...
                    <p class="text-muted-foreground text-xs">
                        ...
                        <template v-if="b.endsAt">
                            · hasta {{ b.endsAt }}</template
                        >
                        <template v-if="b.lockedPrice"
                            ><br />Paga {{ b.lockedPrice }}</template
                        >
                    </p>
                </div>
                ...
                <div class="flex w-full items-center gap-1.5 sm:w-auto">
                    <Link
                        v-if="b.finished"
                        :href="businessShow(b.slug)"
                        class="text-muted-foreground hover:text-foreground grid size-9 place-items-center rounded-full"
                    ...
                    <Button
                        variant="outline"
                        size="sm"
                        class="flex-1 rounded-full sm:flex-none"
                        @click="founderTarget = b"
                    >
    ...
    <Dialog :open="target !== null" @update:open="(v) => !v && (target = null)">
        <DialogContent class="sm:max-w-md">
            <DialogHeader>
                ...
                <DialogDescription>
                    Útil para regalar Pro o activar un pago que llegó por
                    transferencia. Toma las condiciones de hoy del plan elegido.
                </DialogDescription>
            </DialogHeader>
            ...
            <form id="assign-plan" class="grid gap-4" @submit.prevent="assign">
                ...
                <div class="grid gap-1.5">
                    ...
                    <input
                        ...
                        v-model="form.note"
                        maxlength="200"
                        placeholder="Por ejemplo: transferencia BI #123456"
                        class="border-input bg-card focus-visible:border-ring focus-visible:ring-ring/50 h-11 rounded-md border px-3 text-base outline-none focus-visible:ring-[3px] md:text-sm"
                    />
                    <p class="text-muted-foreground text-xs">
                        Queda en el registro de cambios.
                    </p>
                </div>
            </form>
    ...
    <Dialog
        ...
        @update:open="(v) => !v && (founderTarget = null)"
    >
        <DialogContent class="sm:max-w-md">
            <DialogHeader>
                <DialogTitle>{{
                    founderTarget?.founder
                        ? `¿Quitar a ${founderTarget?.name} de fundadores?`
                        : `¿Hacer fundador a ${founderTarget?.name}?`
                }}</DialogTitle>
                <DialogDescription>
                    <template v-if="founderTarget?.founder">
                        Dejará de tener todo sin límites y se le aplicarán los
                        de su plan (salvo mientras "Todo libre" esté encendido).
                        Lo que ya publicó no se borra.
                    </template>
                    <template v-else>
                        Tendrá todo sin límites para siempre, sin importar su
                        plan. "Promocionado" sigue dependiendo de su plan.
                    </template>
                </DialogDescription>
            </DialogHeader>
            <DialogFooter class="gap-2">
                ...
                <Button
                    class="rounded-full"
                    :variant="
                        founderTarget?.founder ? 'destructive' : 'default'
                    "
                    :disabled="togglingFounder"
                    @click="toggleFounder"
                >
                    <Spinner v-if="togglingFounder" />
                    {{
                        founderTarget?.founder
                            ? 'Quitar fundador'
                            : 'Hacer fundador'
