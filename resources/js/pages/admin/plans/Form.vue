...
function submit(): void {
    ...
    if (props.plan) {
        form.transform(({ code: _code, ...data }) => data).put(
            update(props.plan.id).url,
            options,
        );
    } else {
        form.transform(({ apply_to_current: _apply, ...data }) => data).post(
            store().url,
            options,
        );
    }
...
<template>
    ...
    <form
        class="mx-auto flex w-full max-w-3xl flex-col gap-5 px-4 py-5 pb-28 md:py-8"
    ...
    >
        <div>
            ...
            <h1 class="font-display mt-2 text-3xl font-extrabold">
                {{ plan ? `Editar ${plan.name}` : 'Crear plan' }}
            </h1>
        </div>

        <InputError
            :message="(page.props.errors as Record<string, string>).plan"
        />
        ...
        <section class="bg-card grid gap-4 rounded-2xl border p-5 md:p-6">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div class="grid gap-1.5">
                    ...
                    <Input
                        ...
                        required
                        maxlength="60"
                        placeholder="Por ejemplo: Emprende"
                        class="h-11"
                    />
                ...
                <div class="grid gap-1.5">
                    ...
                    <Input
                        ...
                        placeholder="emprende"
                        class="h-11 font-mono"
                    />
                    <p
                        v-else
                        class="bg-muted text-muted-foreground flex h-11 items-center rounded-md px-3 font-mono text-sm"
                    >
                        {{ plan?.code }}
                    </p>
                    <p class="text-muted-foreground text-xs">
                        {{
                            editing
                                ? 'El código no cambia: la app lo usa por dentro.'
            ...
            <div class="grid gap-1.5">
                ...
                <textarea
                    ...
                    rows="2"
                    :class="textareaClass"
                    placeholder="Una frase que explique a quién le conviene"
                />
                ...
                <label class="flex min-h-11 flex-1 items-center gap-3">
                    ...
                    <span class="text-sm">
                        ...
                        <span class="text-muted-foreground block text-xs"
                            >Inactivo: nadie nuevo puede elegirlo; quien ya lo
                            tiene lo conserva.</span
                        >
                    </span>
                </label>
            </div>
        ...
        <section class="bg-card rounded-2xl border p-5 md:p-6">
            ...
            <p class="text-muted-foreground text-sm">
                No aplican a los negocios fundadores ni mientras "Todo libre"
                esté encendido.
            </p>
            ...
            <ul class="divide-border mt-3 divide-y">
                ...
                >
                    ...
                    <label class="flex items-center gap-2 text-sm">
                        <Checkbox
                            :model-value="form[l.key] === null"
                            @update:model-value="
                                (v) => setUnlimited(l.key, v === true)
                            "
                        />
                        Sin límite
                    </label>
        ...
        <section class="bg-card rounded-2xl border p-5 md:p-6">
            ...
            <ul class="divide-border mt-2 divide-y">
                ...
                >
                    ...
                    <ToggleSwitch
                        v-model="form[c.key as PlanCapabilityKey]"
                        :label="c.label"
                    />
                </li>
            </ul>
        ...
        <section class="bg-card rounded-2xl border p-5 md:p-6">
            <div class="flex flex-wrap items-start justify-between gap-2">
                <div>
                    ...
                    <p class="text-muted-foreground text-sm">
                        En dólares, pagados por transferencia. Sin precios, el
                        plan no tiene costo.
                    </p>
                </div>
                <Button
            ...
            >
                <li
                    ...
                    :key="p.id ?? `new-${i}`"
                    class="bg-muted/50 flex flex-wrap items-end gap-3 rounded-xl p-3"
                >
                    <div class="grid gap-1">
                        <Label :for="`months-${i}`" class="text-xs"
                            >Duración</Label
                        >
                        <select
                            :id="`months-${i}`"
                            v-model.number="p.interval_months"
                            :class="[selectClass, 'w-32']"
                        >
                            <option v-for="m in intervals" :key="m" :value="m">
                                {{ m === 1 ? '1 mes' : `${m} meses` }}
                            </option>
                        </select>
                    </div>
                    <div class="grid gap-1">
                        <Label :for="`price-${i}`" class="text-xs"
                            >Precio (USD)</Label
                        >
                        <div class="relative">
                            <span
                            ...
                            <Input
                                ...
                                step="0.01"
                                inputmode="decimal"
                                class="h-11 w-28 pl-6 tabular-nums"
                            />
                        </div>
                    </div>
                    <p class="text-muted-foreground pb-3 text-xs tabular-nums">
                        {{ p.interval_months > 1 ? perMonth(p) : '' }}
                    </p>
                    <label class="ml-auto flex items-center gap-2 pb-2 text-sm">
                        <ToggleSwitch
                            v-model="p.is_active"
                            :label="`Precio de ${p.interval_months} meses activo`"
                        />
                        <span class="hidden sm:inline">{{
                            p.is_active ? 'A la venta' : 'Oculto'
                        }}</span>
                    </label>
                    <Button
                        type="button"
                        ...
                        :aria-label="`Quitar precio de ${p.interval_months} meses`"
                    ...
                    <InputError
                        :message="
                            errors[`prices.${i}.interval_months`] ??
            ...
            <p v-if="editing" class="text-muted-foreground mt-3 text-xs">
                Quien ya paga conserva el precio con el que entró. Un precio con
                pagos registrados no se borra: queda oculto.
            </p>
        </section>

        <!-- Quién recibe el cambio -->
        <section
            v-if="editing && current > 0"
            class="bg-wait-soft rounded-2xl p-5"
        >
            <label class="flex items-start gap-3">
                <Checkbox
                    v-model="form.apply_to_current"
                    class="mt-0.5 bg-white dark:bg-transparent"
                />
                <span class="text-sm">
                    <strong
                        >Aplicar también a los {{ current }}
                        {{
                            current === 1
                                ? 'negocio que ya lo usa'
                                : 'negocios que ya lo usan'
                        }}</strong
                    >
                    <span class="mt-0.5 block">
                        Sin marcar, los límites y funciones nuevos solo aplican
                        a quien entre desde ahora. Úsalo para mejorar un plan,
                        no para recortarlo. Su precio no cambia.
                    </span>
                </span>
            </label>
        </section>
        ...
        <!-- Borrar -->
        <section
            v-if="plan && !plan.protected"
            class="flex flex-wrap items-center gap-3 rounded-2xl border border-dashed p-4"
        >
            <p class="text-muted-foreground min-w-0 flex-1 text-sm">
                Solo se puede borrar si ningún negocio lo ha usado. Si ya lo
                usaron, desactívalo.
            </p>
            <Button
                type="button"
                variant="ghost"
                class="text-destructive rounded-full"
        ...
        <div
            class="bg-background/90 fixed inset-x-0 bottom-0 z-20 border-t px-4 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom,0px))] backdrop-blur md:sticky md:-mx-4 md:rounded-b-2xl"
        >
            <div class="mx-auto flex max-w-3xl items-center justify-end gap-2">
                ...
                <Link
                    :href="plansIndex()"
                    class="text-muted-foreground hover:text-foreground px-3 text-sm font-bold"
                    >Cancelar</Link
                >
                <Button
                    type="submit"
    ...
    <Dialog v-model:open="confirmDelete">
        <DialogContent class="sm:max-w-md">
            <DialogHeader>
                ...
                <DialogDescription
                    >Se borra con sus precios. Esto no se puede
                    deshacer.</DialogDescription
                >
            </DialogHeader>
