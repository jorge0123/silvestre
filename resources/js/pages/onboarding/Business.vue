...
<template>
    ...
    <div class="bg-background min-h-svh">
        <header
            class="bg-background/85 border-border sticky top-0 z-20 border-b backdrop-blur"
        >
            <div
                class="mx-auto flex max-w-5xl items-center gap-3 px-4 py-3 sm:px-6"
            >
                <AppLogoIcon class="size-9 flex-none" />
                <div class="min-w-0 flex-1">
                    <p class="font-display truncate leading-tight font-bold">
                        Abrir mi negocio
                    </p>
                    <p class="text-muted-foreground text-xs">
                        Paso {{ index + 1 }} de {{ steps.length }} · se guarda
                        solo
                    </p>
                </div>
                <ThemeToggle />
        ...
        <div class="mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:py-10">
            ...
            <nav aria-label="Pasos" class="mb-8 hidden md:block">
                <ol class="flex items-center gap-1">
                    ...
                    >
                        ...
                        >
                            <span
                                class="bg-brand text-brand-foreground grid size-6 place-items-center rounded-full"
                            >
                                <Check class="size-3.5" stroke-width="3" />
                            </span>
                            <span
                                class="text-muted-foreground group-hover:text-foreground"
                                >{{ onboardingGuide[key].short }}</span
                            >
                        </Link>
                        <span
                            v-else
            ...
            <div
                class="grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-12"
            >
                <main
                    :key="step"
                    class="animate-in fade-in slide-in-from-bottom-2 min-w-0 duration-300"
                >
                    <p
                        class="text-primary text-xs font-bold tracking-[0.14em] uppercase"
                    >
                        Paso {{ index + 1 }} · {{ guide.short }}
                    </p>
                    <h1
                        class="font-display mt-2 text-[clamp(1.8rem,4vw,2.4rem)] leading-[1.08] font-extrabold"
                    >
                        {{ guide.title }}
                    </h1>
                    ...
                    <div class="bg-brand-soft mt-5 rounded-xl lg:hidden">
                        ...
                        <div
                            v-if="guideOpen"
                            class="text-foreground/85 space-y-2 px-4 pb-4 text-sm"
                        >
                            <p><strong>Qué es:</strong> {{ guide.what }}</p>
                            <p>
                                <strong>Por qué lo pedimos:</strong>
                                {{ guide.why }}
                            </p>
                ...
                <aside class="hidden lg:block">
                    <div class="bg-card sticky top-28 rounded-2xl border p-5">
                        <p
                            class="text-brand flex items-center gap-2 text-xs font-bold tracking-[0.14em] uppercase"
                        >
                            <Lightbulb class="size-4" /> Guía
                        </p>
                        <h2 class="font-display mt-3 text-sm font-bold">
                            Qué es
                        </h2>
                        <p
                        ...
                        <h2 class="font-display mt-4 text-sm font-bold">
                            Por qué lo pedimos
                        </h2>
                        <p
                            class="text-muted-foreground mt-1 text-sm leading-relaxed"
                        ...
                        <ul class="mt-2 space-y-2">
                            <li
                                ...
                                :key="tip"
                                class="text-muted-foreground flex gap-2 text-sm leading-relaxed"
                            >
                                <span
                                    class="bg-primary mt-2 size-1.5 flex-none rounded-full"
                                />
                                {{ tip }}
                            </li>
                        </ul>
